## Sample Batch WOLA program for z/OS Connect Enterprise Edition (EE)

A COBOL batch program which demonstrate how to use the WebSphere Optimized Local Adapters (WOLA) native 
...
* Customize the uploaded copy of **baqls2js.jcl** for your environment and submit. Additional
instructions are provided in the sample JCL. The expected return code is **0**.

### Configuring

* Create a directory (if not done yet) called **dataXform** under your server path.
```
/var/zosconnect/servers/<server-name>/dataXform
```
* Create the following subdirectories under that
```
/var/zosconnect/servers/<server-name>/dataXform/json
/var/zosconnect/servers/<server-name>/dataXform/bind
/var/zosconnect/servers/<server-name>/dataXform/sars
```
* Copy the files generated by the **BAQLS2JS** job to the directories you just created. At this point, you should have:
```
/var/zosconnect/servers/<server-name>/dataXform
|
+--/json
|       /CobolService_request.json
|       /CobolService_response.json
|
+--/bind
|       /CobolService.wsbind
|
...
```
* Setup definitions for the COBOL batch sample in server.xml. The server.xml should have the following entries added:
```
...
<zosconnect_zosConnectDataXform bindFileLoc="/var/zosconnect/servers/<server-name>/dataXform/bind" 
    bindFileSuffix=".wsbind" 
    id="xformJSON2Byte" 
    requestSchemaLoc="/var/zosconnect/servers/<server-name>/dataXform/json" 
    requestSchemaSuffix=".json" 
    responseSchemaLoc="/var/zosconnect/servers/<server-name>/dataXform/json" 
